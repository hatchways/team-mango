...
function Alert(props) { //MuiAlert is a standard original component from Material-UI. Alert is a styled one with following properties
  return <MuiAlert elevation={6} variant="filled" {...props} />;
}

const backgroundStyle = (theme) => ({//styling for this page. It will be used with withStyles at the end of the file
  root: {
    flexGrow: 1,
    height: "100vh",
  },
...
const labels = { //array for Rating component
  1: "JUNIOR",
  2: "ADVANCED JUNIOR",
  3: "INTERMEDIATE",
  4: "SENIOR",
...
function Background(props){ //the function component for the background page is declared here
  ...
  const handleClick = () => { //the method to handle "next step" button being clicked
    setOpenSnack((experience === "" ) ? true : false );
  }; 
  const handleClose = (event, reason) => { //the method to handle Snack "x" button being clicked
    if (reason === "clickaway") {
      return;
    }
    setOpenSnack(false);
  };
  const languageChange = (e) => { //change the value(state) of language when it is selected
    setLanguage(e.target.value);
  };
  const experienceChange = (e) => { //change the value(state) of experience when it is selected
      ...
      <div className={classes.root}> {/*use classNmae instead of class in React. div is a HTML tag */}
        <Paper className={classes.paper} elevation={0}> {/*Paper and Grid are Material-UI components*/}
          <Grid container spacing={0}>
            <Grid item>
              <Grid className={classes.image}></Grid> {/*root, paper, image, ... are styled above*/}
            </Grid> {/*Grid is usually used for the format of the layout*/}
